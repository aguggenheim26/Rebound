...
export default function SignupPage() {
  ...
  return (
    <div className="min-h-screen bg-cream flex items-center justify-center p-4">
      <div className="fixed top-0 right-0 w-80 h-80 rounded-full blur-3xl opacity-20 pointer-events-none" style={{ background: "#FFCECE" }} />
      <div className="fixed bottom-0 left-0 w-96 h-96 rounded-full blur-3xl opacity-15 pointer-events-none" style={{ background: "#FFDABB" }} />
      ...
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-[#7A5C48] mt-2">Every story deserves a next chapter ✨</p>
        </div>

        <div className="bg-white rounded-3xl border p-8 shadow-card" style={{ borderColor: "#EDD9C8" }}>
          <h1 className="font-display text-2xl font-bold text-[#2D1B0E] mb-6">Create your account</h1>
          ...
          <form onSubmit={handleSignup} className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-[#7A5C48] mb-1.5">Your name</label>
              <input
                type="text"
                value={name}
          ...
          <p className="text-center text-xs text-[#B08C78] mt-4 leading-relaxed">
            By joining, you agree to our Terms and Privacy Policy.
          </p>

          <p className="text-center text-sm text-[#B08C78] mt-5">
            Already have an account?{" "}
            <Link href="/auth/login" className="text-[#F95F5F] font-semibold hover:underline">
              Sign in
            </Link>
          </p>
        </div>
